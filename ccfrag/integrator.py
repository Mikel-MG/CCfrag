...
# read PDB file
def load_pdb_2_df(input_pdb: Path) -> pd.DataFrame:
...
def load_json_data(target_pdb: Path) -> dict:
    target_json = get_json_from_pdb(str(target_pdb))
    with open(target_json, "r") as inpt:
        data_json = json.load(inpt)
    return data_json


def get_json_from_pdb(pdb_name: str) -> str:
    """
...
def fix_esmfold_pdb(df_pdb: pd.DataFrame) -> pd.DataFrame:
    ...
    ref_chain_resi = df_pdb_fixed.loc[df_pdb_fixed["chain_id"] == ref_chain][
        "residue_number"
    ]
...
def trim_flanking_seq(df_pdb: pd.DataFrame, N_length_flank: int) -> pd.DataFrame:
    df_pdb = df_pdb.copy()
...
def write_updated_pdb(target_pdb: Path, df_pdb_trimmed: pd.DataFrame, output_pdb: Path):
...
def extract_sequence_from_pandaspdb(df_pdb: pd.DataFrame) -> str:
...
def compute_dist(mat_A: np.ndarray, mat_B: np.ndarray) -> np.ndarray:
...
def detect_if_pdb_parallel(df_pdb: pd.DataFrame) -> int:
    # load CA coordinates
    df_pdb = df_pdb.loc[df_pdb.atom_name == "CA"]
    ...
    for chain in list_chains:
        coord_chain = df_pdb.loc[df_pdb["chain_id"] == chain][
...
def run_socket(pdb_file: Path, threshold: float = 7.4):
    run_dssp(pdb_file)
...
def parse_socket(input_socket: Path = Path("./temp.socket")) -> list:
...
def detect_kih(input_pdb: Path, length_fragment: int) -> np.ndarray:
...
def compute_DSSP_helix(input_pdb: Path) -> np.ndarray:
...
def incorporate_kih():
    vec_knobs = detect_kih(Path(target_pdb), fragment_size)
    norm_vec_knobs = (
        vec_knobs / nmer
...
def incorporate_dssp_H():
    mat_heli_construct = compute_DSSP_helix(Path(target_pdb))
...
class Integrator:
    ...
    def run_assembly_analysis(self):
        ...
        df_data_sequence = pd.DataFrame(
            [*full_sequence], columns=pd.Series(["residue_name"])
        )
            ...
            for i_construct, row in df_constructs.iterrows():
                ...
                if prediction_format == "colabfold":
                    ...
                    target_pdb = [
                        ...
                        if str(construct_name) in pdb_file and "rank_001" in pdb_file
                    ][0]
                    df_pdb = load_pdb_2_df(Path(target_pdb))
                    data_json = load_json_data(Path(target_pdb))
                    ...
                    target_pdb = [
                        pdb_file
                        for pdb_file in list_pdb_files
                        if str(construct_name) in pdb_file
                    ][0]
                    df_pdb = load_pdb_2_df(Path(target_pdb))
                    df_pdb = fix_esmfold_pdb(df_pdb)
                    write_updated_pdb(Path(target_pdb), df_pdb, Path("./temp.pdb"))
                    target_pdb = "temp.pdb"
